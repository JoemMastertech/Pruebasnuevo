 ...
 */
export class SupabaseAdapterTS implements ProductRepositoryPort {
  private client: any;
   ...
   */
  private getEnvironmentVariables(): { url: string; anonKey: string } {
    try {
      ...
      if (typeof window !== 'undefined' && (window as any).env) {
        return {
          url: (window as any).env.VITE_SUPABASE_URL || '',
          anonKey: (window as any).env.VITE_SUPABASE_ANON_KEY || ''
        };
      }
   ...
   */
  private async safeExecuteAsync<T>(operation: () => Promise<any>, methodName: string, fallbackValue: T): Promise<T> {
    try {
