 ...
 */
export class ProductDataAdapterTS implements ProductRepositoryPort {
  private productDataAdapter: any;
   ...
   */
  private getExistingAdapter(): any {
    try {
      // Try to get from window
      if (typeof window !== 'undefined' && (window as any).ProductDataAdapter) {
        return new (window as any).ProductDataAdapter();
      }
      
      // Try to get from container
      if (typeof window !== 'undefined' && (window as any).container) {
  ...
  async findByName(name: string): Promise<Product | null> {
    try {
      ...
      return allProducts.find(product => 
        product.name.value.toLowerCase().includes(name.toLowerCase())
      ) || null;
    } catch (error) {
  ...
  async search(query: string): Promise<Product[]> {
    try {
      ...
      return allProducts.filter(product => 
        product.name.value.toLowerCase().includes(query.toLowerCase()) ||
   ...
   */
  async getDrinkOptions(product: Product): Promise<any[]> {
    try {
   ...
   */
  private mapRawProductsToDomain(rawProducts: any[]): Product[] {
    return rawProducts.map(rawProduct => this.mapRawProductToDomain(rawProduct)).filter(Boolean) as Product[];
  }

  /**
   * Map single raw product to domain entity
   */
  private mapRawProductToDomain(rawProduct: any): Product | null {
    try {
      ...
      return new Product(
        new ProductId(String(id)),
        new ProductName(name),
        new ProductCategory(category),
        new Money(price),
        rawProduct.descripcion || rawProduct.description || rawProduct.ingredientes || rawProduct.ingredients || ''
      );
    } catch (error) {
