...
const ProductCategory_1 = require("../../../Domain/ValueObjects/ProductCategory");
class ProductController {
    constructor(getProductsUseCase, getProductByIdUseCase, presenter) {
    ...
    async getProductById(productId) {
        try {
            ...
            const id = new ProductId_1.ProductId(productId);
    ...
    async getProductsByCategory(categoryName) {
        try {
            ...
            if (result.isSuccess()) {
                ...
                const category = new ProductCategory_1.ProductCategory(categoryName);
                const filteredProducts = allProducts.filter(product => product.getCategory().value === category.value);
    ...
    async searchProducts(searchTerm) {
        try {
            ...
            if (result.isSuccess()) {
                const allProducts = result.getValue();
                const searchResults = allProducts.filter(product => product.getName().toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    async filterProducts(filter) {
        try {
            ...
            if (result.isSuccess()) {
                ...
                if (filter.category) {
                    const category = new ProductCategory_1.ProductCategory(filter.category);
                    filteredProducts = filteredProducts.filter(product => product.getCategory().value === category.value);
                }
                // Filtrar por término de búsqueda
                if (filter.searchTerm) {
                    filteredProducts = filteredProducts.filter(product => product.getName().toLowerCase().includes(filter.searchTerm.toLowerCase()) ||
