...
class Phase3GlobalExposureTest {
  ...
  async testBemBaseCssLoaded() {
    ...
    const bemBaseLoaded = stylesheets.some(sheet => 
      sheet.href && sheet.href.includes('_bem-base.css')
    );
  ...
  generateSummary() {
    ...
    Object.values(this.results.adapters).forEach(result => {
      totalTests++;
      if (result.resolvable) passedTests++;
    });
    ...
    if (this.results.global.containerAvailable) passedTests++;
    totalTests++;
    
    if (this.results.global.bootstrapAvailable) passedTests++;
    totalTests++;
    
    if (this.results.global.bemBaseCssLoaded) passedTests++;
    totalTests++;
