...
class OrderSystem {
    ...
    handleBoostOption(target) {
        ...
        if (target.checked) {
            ...
            document.querySelectorAll('.exclusive-option-group .drink-option-container .counter-btn, .exclusive-option-group .drink-option-container .count-display')
                .forEach(el => {
                if (el && el.classList) {
                    if (el.classList.contains('counter-btn'))
                        el.disabled = true;
                    if (el.classList.contains('count-display'))
                        el.textContent = '0';
                }
            });
        }
    ...
    _updateHamburgerMenuButton(isActive) {
        ...
        const createOrderBtn = Array.from(hamburgerButtons).find(btn => btn.getAttribute('data-action') === 'createOrder');
        if (createOrderBtn) {
    ...
    getPriceType(row, clickedElement) {
        ...
        if (clickedElement.dataset.field)
            return clickedElement.dataset.field;
        ...
        if (headerText.includes('BOTELLA'))
            return CONSTANTS.PRICE_TYPES.BOTTLE;
        if (headerText.includes('LITRO'))
            return CONSTANTS.PRICE_TYPES.LITER;
        if (headerText.includes('COPA'))
            return CONSTANTS.PRICE_TYPES.CUP;
        return 'precio';
    }
    isBottleProduct(row) {
    ...
    updateTotalJagerDrinkCount() {
        ...
        document.querySelectorAll('.exclusive-option-group .drink-option-container .counter-btn')
            .forEach(btn => {
            if (btn.textContent === '+')
                btn.disabled = boostCheck.checked || total >= this.maxDrinkCount;
        });
    }
    ...
    _validateSpecialBottleRules(isJuice, totalJuices, totalRefrescos) {
        return OrderSystemValidations.validateSpecialBottleRules(isJuice, totalJuices, totalRefrescos, this.bottleCategory, this.currentProduct?.name);
    }
    ...
    _isOnlySodaCategory() {
        ...
        if (!currentOptions || !currentOptions.drinkOptions)
            return false;
    ...
    _validateSpecialDrinkLimits(isJuice, totalJuices, totalRefrescos) {
        return OrderSystemValidations.validateSpecialDrinkLimits(isJuice, totalJuices, totalRefrescos, this.bottleCategory, this.currentProduct?.name);
    }
    ...
    _setupFoodModal() {
        ...
        if (ingredientsContainer)
            ingredientsContainer.className = 'input-container-hidden';
    ...
    _showIngredientsInput() {
        ...
        if (ingredientsChoice)
            ingredientsChoice.className = 'choice-hidden';
    }
    ...
    _showGarnishInput() {
        ...
        if (garnishChoice)
            garnishChoice.className = 'choice-hidden';
    }
    ...
    _validateCookingTerm() {
        ...
        if (!isValid) {
            OrderSystemValidations.showValidationModal('Por favor seleccione un término de cocción primero', this._createSimpleModal.bind(this));
        }
        return isValid;
    }
    ...
    completeOrder() {
        ...
        if (currentOrderItems.length === 0) {
            OrderSystemValidations.showValidationModal('La orden está vacía. Por favor agregue productos.', this._createSimpleModal.bind(this));
            return;
        }
    ...
    getLiquorType(productName) {
        ...
        const normalizedName = productName
            ...
            .toUpperCase();
        if (normalizedName.includes('MALIBU'))
            return 'RON';
        if (normalizedName.includes('TRIPAS DE MAGUEY'))
            return 'MEZCAL';
        ...
        if (normalizedName.includes("RON"))
            return 'RON';
        if (normalizedName.includes("TEQUILA"))
            return 'TEQUILA';
        if (normalizedName.includes("WHISKY"))
            return 'WHISKY';
        if (normalizedName.includes("VODKA"))
            return 'VODKA';
        if (normalizedName.includes("BRANDY"))
            return 'BRANDY';
        if (normalizedName.includes("GINEBRA") || normalizedName.includes("GIN"))
            return 'GINEBRA';
        if (normalizedName.includes("MEZCAL"))
            return 'MEZCAL';
        if (normalizedName.includes("COGNAC"))
            return 'COGNAC';
        if (normalizedName.includes("DIGESTIVO"))
            return 'DIGESTIVOS';
        if (normalizedName.includes("ESPUMOSO"))
            return 'ESPUMOSOS';
    ...
    deleteOrder(orderId) {
        this._showConfirmationModal('¿Está seguro de que desea eliminar esta orden?', 'Esta acción moverá la orden al historial.', () => {
    ...
    showOrdersScreen() {
        ...
        if (elements.ordersScreen) {
            ...
            if (historyButton)
                historyButton.textContent = 'Historial Órdenes';
    ...
    _showHistoryView(button, { ordersList, orderHistoryContainer, ordersScreenTitle }) {
        ...
        if (ordersScreenTitle)
            ordersScreenTitle.textContent = 'Historial de Órdenes';
        if (ordersList) {
    ...
    _showActiveOrdersView(button, { ordersList, orderHistoryContainer, ordersScreenTitle }) {
        ...
        if (ordersScreenTitle)
            ordersScreenTitle.textContent = 'Órdenes Guardadas';
        if (orderHistoryContainer) {
    ...
    deleteFromHistory(orderId) {
        this._showConfirmationModal('¿Está seguro de que desea eliminar esta orden del historial?', 'Esta acción eliminará permanentemente la orden del historial.', () => {
    ...
    promptClearHistory(historyContainer) {
        this._showConfirmationModal('¿Está seguro de que desea limpiar el historial?', 'Esta acción no se puede deshacer.', () => {
...
//# sourceMappingURL=order-system.js.map