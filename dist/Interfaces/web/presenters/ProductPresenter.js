...
export class ProductPresenter {
     ...
     */
    presentProductList(products, context) {
    ...
    updateActiveFilters(filters) {
        ...
        const searchInput = this.filterContainer.querySelector('.product-filters__search-input');
        if (searchInput && filters.searchTerm !== undefined) {
            searchInput.value = filters.searchTerm;
        }
        // Actualizar categoría seleccionada
        if (filters.category) {
            const categoryInputs = this.filterContainer.querySelectorAll('input[name="category"]');
        ...
        if (filters.priceRange) {
            const minInput = this.filterContainer.querySelector('.product-filters__price-min');
            const maxInput = this.filterContainer.querySelector('.product-filters__price-max');
            if (minInput)
                minInput.value = filters.priceRange.min.toString();
            if (maxInput)
                maxInput.value = filters.priceRange.max.toString();
        }
    }
}
//# sourceMappingURL=ProductPresenter.js.map