...
export class ValidateProductUseCase {
    ...
    validateDrinkSelection(product, selectedDrinks) {
        ...
        for (const drink of selectedDrinks) {
            if (!drinkOptions.isValidOption(drink.drinkName)) {
                return ValidationResult.failure(`Bebida no válida para este producto: ${drink.drinkName}`);
            }
            if (drink.quantity <= 0) {
                return ValidationResult.failure(`La cantidad de ${drink.drinkName} debe ser mayor a cero`);
            }
        }
        return ValidationResult.success();
    }
    ...
    validateCookingTerm(product, cookingTerm) {
        ...
        if (!validTerms.includes(normalizedTerm)) {
            return ValidationResult.failure(`Término de cocción no válido. Opciones: ${validTerms.join(', ')}`);
        }
        return ValidationResult.success();
    }
}
