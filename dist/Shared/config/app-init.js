    ...
    ensureMainScreenVisible: function (contentType) {
        ...
        setTimeout(() => {
            ...
            if (mainScreen) {
                ...
                const screenElements = Array.from(allElements).filter(el => el.className && (el.className.includes('screen') || el.className.includes('content')));
                window.Logger.info(`[NAVIGATION DEBUG] Available screen/content elements:`, screenElements.map(el => ({
                    tag: el.tagName,
                    id: el.id,
