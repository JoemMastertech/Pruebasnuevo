...
exports.ValidateProductUseCase = void 0;
...
class ValidateProductUseCase {
    constructor(productRepository, drinkRules) {
     ...
     */
    async getDrinkOptionsForProduct(productName) {
     ...
     */
    async validateDrinkSelectionOnly(productName, selectedDrinks) {
        const product = await this.productRepository.findByName(productName);
        if (!product) {
    ...
    validateQuantity(quantity) {
        if (quantity <= 0) {
            return DrinkRulesPort_js_1.ValidationResult.failure('La cantidad debe ser mayor a cero');
        }
        if (!Number.isInteger(quantity)) {
            return DrinkRulesPort_js_1.ValidationResult.failure('La cantidad debe ser un número entero');
        }
        if (quantity > 50) {
            return DrinkRulesPort_js_1.ValidationResult.warning('Cantidad muy alta, verifique si es correcta');
        }
    ...
    validateDrinkSelection(product, selectedDrinks) {
        ...
        for (const drink of selectedDrinks) {
            if (!drinkOptions.isValidOption(drink.drinkName)) {
                return DrinkRulesPort_js_1.ValidationResult.failure(`Bebida no válida para este producto: ${drink.drinkName}`);
            }
            if (drink.quantity <= 0) {
                return DrinkRulesPort_js_1.ValidationResult.failure(`La cantidad de ${drink.drinkName} debe ser mayor a cero`);
            }
        }
        return DrinkRulesPort_js_1.ValidationResult.success();
    }
     ...
     */
    validateCookingTerm(product, cookingTerm) {
        if (!product.isFood()) {
            return DrinkRulesPort_js_1.ValidationResult.failure('Solo los alimentos pueden tener término de cocción');
        }
        ...
        if (!validTerms.includes(normalizedTerm)) {
            return DrinkRulesPort_js_1.ValidationResult.failure(`Término de cocción no válido. Opciones: ${validTerms.join(', ')}`);
        }
        return DrinkRulesPort_js_1.ValidationResult.success();
    }
}
exports.ValidateProductUseCase = ValidateProductUseCase;
