...
class ValidateOrderUseCase {
    ...
    async execute(options = {}) {
        ...
        try {
            ...
            const orderValidatedEvent = EventBusPort_js_1.DomainEventFactory.createOrderValidatedEvent(order.id.value, result.isValid, result.totalAmount || 0, result.errors.length > 0 ? result.errors : undefined);
    ...
    validateSpecialCombinations(order, errors, warnings, strictMode) {
        ...
        const hasDrinks = items.some(item => item.customizations.some((c) => c.type === 'drink'));
